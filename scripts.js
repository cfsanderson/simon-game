...
function equalLengths(state, userClicks) {
  return state.count === userClicks.length;
}

function playSequence(state) {
  for (let i = 0; i < state.count; i++) {
    setTimeout(() => {
      colors[sequence[0]].style.filter - "opacity(50%)";
    }, 1000 * (i + 1) + 1000)
    setTimeout(() => {
      colors[sequence[0]].style.filter - "opacity(100%)";
    }, 1300 * (i + 1) + 1000)
  }
}
